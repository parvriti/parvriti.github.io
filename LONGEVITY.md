...
_Last audited: 2026-09-18 (v150)._

---

## The yearly ~10-minute check
1. Open **dev.html** → glance at the **storage** gauge (base64 media fills it), and the
   **Faults** card: anything new from either phone? Mute what turns out to be harmless.
...
- **Faults reach you from BOTH phones (v150).** `common.js` quietly records any error nobody
  caught, a script or Firebase failing to load, and real Firestore failures (never offline
  blips). Each phone keeps them in a small outbox and hands them to `faults/<person>` at most
  every 6 hours. The Developer panel's **Faults** card lists them; a new one lights the Settings
  dot amber (Parv only, never Riti). **Mute** anything harmless from the card and it never
  lights the dot again, so noise never needs a code change. Needs the `/faults` rule published.
...
- You're on **FCM v1 (the current API)**, so the legacy-FCM shutdowns do **not** affect you.
...
- **The trap:** the SDK is **not** in the offline precache, and every deploy clears the old cache.
  So if Google drops these files, both installed apps keep working **until our next deploy**, and
  that deploy breaks both phones at once. `scripts/release.sh preflight` now refuses a bump unless
  Google still serves every SDK and font file, so that deploy cannot happen by accident.
- **If it breaks:** data/auth won't load. The app still opens its static pages, but nobody can
  sign in, so the Developer panel cannot open and nothing is delivered; each phone keeps a
  "Firebase did not load" fault that only shows up once Firebase loads again. So the preflight
  check above is the real guard, not the Faults card. **Fix:** bump the SDK version in the 7 HTML files **and**
  the two `importScripts` lines in `firebase-messaging-sw.js`, or push notifications break.
  Preflight checks all of them carry the same version.
...
- **Fault inbox:** `faults/parv` + `faults/riti` in Firestore (rule in `firestore-rules/`), read
  only by the Developer panel and Parv's Settings dot.
- **Version discipline:** every ship bumps `?v=NN` across the 7 HTML + `sw.js` (CORE + CACHE) +
  `settings.js` VERSION, then commit + `git tag vNN` + push. `scripts/release.sh bump NN` does
  the bump and runs the preflight; `bash tests/run.sh` runs the regression suites.
